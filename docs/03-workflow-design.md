...
五个阶段在 Quality Gate 通过后都必须等待用户审批，批准后才能锁定产物并生成交接包。

### `AUTONOMOUS`

阶段 Quality Gate 通过后自动锁定产物并生成交接包，不创建阶段审批请求。用户仍可随时暂停或停止工作流。

`AUTONOMOUS` 只取消阶段交接审批。任何角色申请默认能力之外的工具、命令或项目内路径时，仍必须创建 `CapabilityRequest` 并弹窗由用户批准。
...
仅当问题属于部署文档或允许生成的部署配置文件时，Deployer 可以在本阶段重试。第一版不运行本地打包、Docker 构建或真实部署；代码和构建配置问题应返回 Builder。
